...
function cliAppend( id,fac,url,city,stat,src ) {
    var row = [url,id,city,fac,stat,src];
	srslist+=id+",";
...
function cliAsString( i ) {
    var a = fulist[i];
    var s = "<td><a href=\""+a[0]+"?srs="+srslist.replace("undefined",'')+"\">"+a[1]+"</a></td><td>"+a[2]+"</td><td>"+a[3]+"</td><td>"+a[4]+" "+a[5]+" "+a[16]+"</td>";
    return s;
}
    ...
    $.getJSON(url, function(result, status) {
        if( status == "success" && result.code == 200 ) {
            $.each(result.report, function(k,v) {
                cliAppend(  v.cli_id, v.ctct_name, v.cliurl, v.city_state, v.st_name, v.cli_source );
            });
            fuBody(); fuPagesSetup();
            fuStoreState(null);
        }
