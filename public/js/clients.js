// script for view/pinnacle/cleints/index.phtml
...
function cliAppend( id,fac,url,city,stat,src,pri ) {
    var row = [url,id,city,fac,stat,src,pri];
	srslist+=id+",";
    return fulist.push( row );
}

function cliAsString( i ) {
    var a = fulist[i];
	if(a[6]==1)
		var s = "<td><a href=\""+a[0]+"?srs="+srslist.replace("undefined",'')+"\">"+a[1]+"</a></td><td>"+a[2]+" <span style='color:red; font-weight:bold'>*P*</span></td><td>"+a[3]+"</td><td>"+a[4]+" "+a[5]+" </td>";
    else
		var s = "<td><a href=\""+a[0]+"?srs="+srslist.replace("undefined",'')+"\">"+a[1]+"</a></td><td>"+a[2]+"</td><td>"+a[3]+"</td><td>"+a[4]+" "+a[5]+" </td>";
    return s;
}
...
function fuPagesSetup() {
    if( futotal ) {
        var s = '<span class="pagination"><ul><li id="li-cli-prev"><a href="#" id="cli-prev" title="Previous Page"><img src="/public/img/images/ARW06LT.png" alt="Prev Page"> </a></li> ';
        var sn = '<li id="li-cli-next"><a href="#" id="cli-next" title="Next Page"><img src="/public/img/images/ARW06RT.png" alt="Next Page"></a></li></span>';
        var s1 = '<li id="li-cli-1st"><a href="#" id="cli-1st">1</a></li> ';
        var st = '<li id="li-cli-last"><a href="#" id="cli-last">'+futotal+"</a></li>";
        var curr = "cli-1st";
        ...
        if( futotal == 1 ) st = "";
            ...
            for(var i=minp; i<=maxp; i++) {
                ...
                s += '<li id="li-cli-'+i+'th"><a href="#" id="cli-'+i+'th">'+i+"</a></li>";
            }
            if( maxp < futotal-1 ) s += shel;
            if( fucp == futotal ) curr = "cli-last";
        }
        else if( fucp == futotal ) curr = "cli-last";

        s += st + sn;
        $(fupages).html(s);
        $("#cli-1st").off("click").click(function(){ fuFetch(1) });
        if( st != "" ) $("#cli-last").off("click").click(function(){ fuFetch(futotal) });
        $("#cli-prev").off("click").click(function(){ fuFetch(fucp - 1) });
        $("#cli-next").off("click").click(function(){ fuFetch(fucp + 1) });
    ...
    $.getJSON(url, function(result, status) {
        if( status == "success" && result.code == 200 ) {
            $.each(result.report, function(k,v) {
                cliAppend(  v.cli_id, v.ctct_name, v.cliurl, v.city_state, v.st_name, v.cli_source, v.primary_record );
            });
            fuBody(); fuPagesSetup();
            fuStoreState(null);
        }
...
function cliInit( id, id2, url, redir, url2 ) {
    fuid = id; fupages = id2; furl = url; fustate = "fustoreClients";
    murl = url2;
