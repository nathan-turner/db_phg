...
<program>
  ...
  <xsl:apply-templates select="tobacco" />
  ...
  <xsl:apply-templates select="360-degree" />
...
<xsl:template match="int_model" >
  <int_model label="{@label}"><xsl:value-of select="." /></int_model>
</xsl:template>
<xsl:template match="prog_length" >
  <prog_length label="{@label}" ampfile="{@ampfile}"><xsl:value-of select="." /></prog_length>
</xsl:template>
...
<xsl:template match="MPH_MBA" >
  <MPH_MBA label="{@label}"><xsl:value-of select="." /></MPH_MBA>
</xsl:template>
<xsl:template match="PhD" >
  <PhD label="{@label}"><xsl:value-of select="." /></PhD>
</xsl:template>
