<?php
// module/Pinnacle/src/Pinnacle/Controller/CalendarController.php:
...
class CalendarController extends AbstractActionController
{
    ...
    public function indexAction() {
		...
		$act = $this->getCalendarTable()->getActivities();
		$act2 = $this->getCalendarTable()->getUserActivities('');
		
		$userid = $_COOKIE["phguid"];
		$dates = $this->getCalendarTable()->getMonthDates($userid, '');
		//create appt submit
		if(isset($_POST["sched_submit_btn"]))
		{
			$result = $this->getCalendarTable()->addActivity($_POST, $identity);
			if($result)
				$messages = "Your activity has been created</p>";
		}
		
        if(isset($_POST["sched_edit_btn"]))
		{
			$result = $this->getCalendarTable()->saveActivity($_POST, $identity);
			if($result)
				$messages = "Your activity has been edited</p>";
		}
        return array('phguser' => $identity, 'users'=>$users, 'act'=>$act, 'userid'=>$userid, 'dates'=>$dates,
	...
	public function monthAction() {
        ...
        return array('phguser' => $identity, 'users'=>$users, 'act'=>$act, 'userid'=>$userid,
            'part' => $part, 'messages' => $messages );
    }
    ...
    public function monmorAction() {
	 ...
	 public function myhotlistsAction() {
        $identity = $this->getServiceLocator()
            ->get('Application\Model\LoginService')->checkAuth();
        if (!$identity) {
            return $this->redirect()->toRoute('login');
		...
		if($_COOKIE["dept"]=="R")
			$market=false;
		elseif($_COOKIE["dept"]=="M")
			$market=true;
		if($_COOKIE["dept"]=="A")
			$allctr = true;
//$market=true;
        $table = $this->getCalendarTable()->getHotLists($market, $filter);
        
        //return array('phguser' => $identity, 'report' => $report, 'messages' => $messages, 'table'=>$table );
		$viewModel = new ViewModel(array('phguser' => $identity, 'report' => $report, 'messages' => $messages, 'table'=>$table ));
