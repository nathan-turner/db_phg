...
class PlacementTable extends ReportTable
{
    ...
    */
	...
	public function getCalls($part='') {         
            $userid = $_COOKIE["phguid"];			
		if($part=='prev')
			$result = $this->adapter->query("select call_emp_id, call_numin, call_numout, sum(call_numin+call_numout) as sumout, avg(call_timein+call_timeout) as timout from tcalls where (MONTH(call_date)=MONTH(SUBDATE(NOW(), INTERVAL 1 MONTH)) AND YEAR(call_date)=YEAR(SUBDATE(NOW(), INTERVAL 1 MONTH))) and WEEKDAY(call_date) not in (5,6) group by call_emp_id", array());
		else
			$result = $this->adapter->query("select call_emp_id, call_numin, call_numout, sum(call_numin+call_numout) as sumout, avg(call_timein+call_timeout) as timout from tcalls where (MONTH(call_date)=MONTH(NOW()) AND YEAR(call_date)=YEAR(NOW())) and WEEKDAY(call_date) not in (5,6) group by call_emp_id", array());
			...
			$i=0;
				foreach ($result as $row) {
					...
					$ar[$i]['call_numin']=$row->call_numin;
					$ar[$i]['call_numout']=$row->call_numout;
					$ar[$i]['call_num']=$row->sumout;
					$i+=1;
				}
			}
	...
	//new get calls for usersumst report
	public function getGoals($part='') {         
            $userid = $_COOKIE["phguid"];		
			$last = date('Y-m-d',strtotime('last day of this month',strtotime("-1 months")));			
			if(date('m')=='01')
				$year = date('Y')-1;
			else 
				$year = date('Y');
			...
			if($part=='prev')
				$result = $this->adapter->query("call GetGoalsActualsNew (DATE_FORMAT(NOW(),'$year-01-01'),? )", array($last));
			else
				$result = $this->adapter->query("call GetGoalsActualsNew (DATE_FORMAT(NOW(),'%Y-01-01'),NOW())", array());
	...
	public function getGoalsTotal($part='') {         
            $userid = $_COOKIE["phguid"];			
			$last = date('Y-m-d',strtotime('last day of this month',strtotime("-1 months")));	
			if(date('m')=='01')
				$year = date('Y')-1;
			...
			if($part=='prev')
				$result = $this->adapter->query("call GetGoalsTotal (DATE_FORMAT(NOW(),'$year-01-01'),?)", array($last));
			else
				$result = $this->adapter->query("call GetGoalsTotal (DATE_FORMAT(NOW(),'%Y-01-01'),NOW())", array());
	...
	//new get passes for usersumst report
	public function getPasses($part='') {         
            $userid = $_COOKIE["phguid"];		
			
			//$result = $this->adapter->query("select * from vphpassesrpt where date_> date_sub(NOW(),interval 30 day)  union select * from vanpassesrpt where date_> date_sub(NOW(),interval 30 day)  order by rec_from, date_ desc", array());
			//$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_> date_sub(NOW(),interval 30 day) group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_> date_sub(NOW(),interval 30 day) group by emp_id  ", array());
			if($part=='prev') //get prev month
				$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_ BETWEEN date_sub(DATE_FORMAT(NOW(),'%Y-%m-01'),interval 1 month) AND DATE_FORMAT(date_sub(LAST_DAY(NOW()), interval 1 month), '%Y-%m-%d 23:59:00') group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_ BETWEEN date_sub(DATE_FORMAT(NOW(),'%Y-%m-01'),interval 1 month) AND  DATE_FORMAT(date_sub(LAST_DAY(NOW()), interval 1 month), '%Y-%m-%d 23:59:00') group by emp_id  ", array());
			else
				$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_ BETWEEN DATE_FORMAT(NOW(),'%Y-%m-01') AND LAST_DAY(NOW()) group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_ BETWEEN DATE_FORMAT(NOW(),'%Y-%m-01') AND  LAST_DAY(NOW()) group by emp_id  ", array());
	...
	//new get passes for usersumst report
	public function getWeeklyPasses($part='') {         
            $userid = $_COOKIE["phguid"];		
			
			//$result = $this->adapter->query("select * from vphpassesrpt where date_> date_sub(NOW(),interval 30 day)  union select * from vanpassesrpt where date_> date_sub(NOW(),interval 30 day)  order by rec_from, date_ desc", array());
			//$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_> date_sub(NOW(),interval 30 day) group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_> date_sub(NOW(),interval 30 day) group by emp_id  ", array());
			if($part=='prev') //get prev month
				$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_ BETWEEN date_sub(date_sub(NOW(), interval 1 month),interval 1 week) AND NOW() group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_ BETWEEN date_sub(date_sub(NOW(), interval 1 month),interval 1 week) AND NOW() group by emp_id  ", array());
			else
				$result = $this->adapter->query("select emp_id, rec_from, count(*) as cnt from vphpassesrpt where date_ BETWEEN date_sub(NOW(),interval 1 week) AND NOW() group by emp_id union select emp_id, rec_from, count(*) as cnt from vanpassesrpt where date_ BETWEEN date_sub(NOW(),interval 1 week) AND NOW() group by emp_id  ", array());
	...
	public function getPltPresents($part='') {         
            $userid = $_COOKIE["phguid"];			
			
			
			if($part=='prev') //get prev month			
				$result = $this->adapter->query("select *, COUNT(*) AS cnt from ltpresents WHERE present_date BETWEEN date_sub(DATE_FORMAT(NOW(),'%Y-%m-01'),interval 1 month) AND DATE_FORMAT(date_sub(LAST_DAY(NOW()), interval 1 month), '%Y-%m-%d 23:59:00') group by user_mod_id",  array());
			else
				$result = $this->adapter->query("select *, COUNT(*) AS cnt from ltpresents WHERE present_date BETWEEN date_sub(NOW(), interval 1 week) AND NOW() group by user_mod_id",  array());
			
				//$result = $this->adapter->query("select *, COUNT(*) AS cnt from ltpresents WHERE present_date BETWEEN DATE_FORMAT(NOW(),'%Y-%m-01') AND LAST_DAY(NOW()) group by user_mod_id",  array());
